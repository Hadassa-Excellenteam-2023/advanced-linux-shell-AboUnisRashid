# 1. Create a folder and the files FileA.txt, FileB.txt...FileF.txt
mkdir subs && touch subs/File{A..F}.txt

# 2. Delete the files FileC.txt to FileF.txt
rm subs/File{C..F}.txt

# 3. Change the name of FileA.txt to FileA.old
mv subs/FileA.txt subs/FileA.old

# 4. Delete all the files in the folder
rm subs/*

# 5. Copy all the files ending in .conf from /etc folder to the subs 
folder
...
# 7. Find which files, with 4-letter names (excluding .conf extension), 
contain the word "user"
grep -l -w "user" subs/????.conf

# 8a. View the last modified file in the folder
ls -lt subs/ | awk 'NR==2{print $NF}'

# 8b. Command to print the last modified file
echo "The last modified file is $(ls -lt subs/ | awk 'NR==2{print $NF}')"

# 9. Create a folder for each group listed in the /etc/group file
cut -d ':' -f 1 /etc/group | xargs -I {} mkdir -p subs/{}
